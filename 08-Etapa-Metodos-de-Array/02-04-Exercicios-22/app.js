...
const namesInAlphabeticalOrder = names.map((item) => item).sort();
...
// console.log({characters, sortedCharacters});
...
const numbersInAscendingOrder = numbers
  .map((number) => number)
...
const firstItemGreaterThan50 = randomNumbers.find((randomNumber) => randomNumber > 50);
...
const peopleInReverseAlphabeticalOrder = people
  .map((person) => person).sort().reverse();
...
const ingredientsMessage = ingredients
  .reduce((accumulator, ingredient, index, array) => {
...
const disneyMovies = ({ distributedBy }) => distributedBy === 'Disney';
const amountOfDisneyMovies = (accumulator, { peopleAmount }) => accumulator + peopleAmount;

const peopleAmountDisneyMovies = topBrazilMovies
  .filter(disneyMovies)
  .reduce(amountOfDisneyMovies, 0);

// console.log(peopleAmountDisneyMovies);
