...
const car = {
  ...
  getColorsMessage () {
    const lastCarColor = this.colors[this.colors.length - 1];
    const carColors = this.colors.join(', ').replace(`, ${lastCarColor}`, ` e ${lastCarColor}`);
    console.log(`O ${this.name} está disponível nas cores ${carColors}.`)
  },
};
...
// const carStatus = car.isRunning = true;
// car.run(carStatus);
...
// const carStatus = car.isRunning = false;
