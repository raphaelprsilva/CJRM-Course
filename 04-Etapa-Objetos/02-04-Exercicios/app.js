...
let car = {
  ...
  getColorsMessage () {
    ...
    const lastCarColor = this.color[this.color.length - 1];
    const carColors = this.color.join(', ').replace(`, ${lastCarColor}`, ` e ${lastCarColor}`);

    console.log(`O ${carName} está disponível nas cores ${carColors}.`);
  }
};
