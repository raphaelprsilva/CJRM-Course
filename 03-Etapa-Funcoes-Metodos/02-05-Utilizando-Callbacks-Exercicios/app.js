...
const convertedToString = convertToString(true);
...
const amountOfCharacters = (myString) => myString.length;

const result = amountOfCharacters('Raphael Silva');
...
const convertToLowerCase = (myString) => myString.toLowerCase();

const convertedString = convertToLowerCase('CHOCOTONE E OVO DE PÁSCOA JUNTOS NO MERCADO EM PLENO FEVEREIRO');
...
// const returnIndex = (character, myString) => myString.indexOf(character);

// const indexReturned = returnIndex('2', '123');

// console.log(indexReturned);
...
// const verifyExistentArgument = (argument, array) => array.includes(argument);


// const myResult = verifyExistentArgument(7, [1, 0, 3, 4, 5]);
...
// const concatenatedArrays = concatArrays([1, 2, 3], [4, 5, 6]);
...
// const myNewArray = removeLastItem([1, 2, 3, 4, 'raphael']);
...
// const verifyArgument = myArgument => myArgument === null;

// const myResult = verifyArgument(null);
...
// const showName = () => console.log('Raphael');
...
// const myFunc = (callbackFn, value) => callbackFn(value);

// const triple = (number) => number * 3;

// console.log(myFunc(triple, 33));
...
//   paragraphs += `<p style="color: red">${paragraph}</p>`;
// };
...
const amountOfLikes = (personsLiked = []) => {
  let message = null;
  const firstName = personsLiked[0];
  const secondName = personsLiked[1];
  const thirdName = personsLiked[2];
  const totalNamesMinusTwo = personsLiked.length - 2;

  if (personsLiked.length === 0) {
    return message = 'Ninguém curtiu isso';
  } else if (personsLiked.length === 1) {
    return message = `${firstName} curtiu isso.`;
  } else if (personsLiked.length === 2) {
    return message = `${firstName} e ${secondName} curtiram isso.`;
  } else if (personsLiked.length === 3) {
    return message = `${firstName}, ${secondName} e ${thirdName} curtiram isso.`;
  } else {
    return message = `${firstName}, ${secondName} e mais ${totalNamesMinusTwo} curtiram isso.`;
  }
};

const myArray = ['Raphael', 'Joana', 'Marcos', 'João'];
const myAmountOfLikes = amountOfLikes(myArray);

console.log(myAmountOfLikes);

// Acertei, mas daria para fazer com 'switch'