...
const my3FavoriteTVShows = ['Filme A', 'Filme B', 'Filme C'];

const sentence = `${my3FavoriteTVShows.join(', ').replace('B, ', 'B e ')}`;
...
const typeOfData = typeof isRandomTVShowIncluded;

const typeSentence = `O tipo de dado que a const "isRandomTVShowIncluded" armazena é: ${typeOfData}.`;
...
const standardLength = 39;
const isBiggerThan39 = String(typeSentence.length > standardLength);
const myRandomSentence = `A string que a "typeSentence" armazena tem mais de ${standardLength} caracteres? ${isBiggerThan39.replace('t', 'T')}.`;

// console.log(myRandomSentence);
...
const isLowerThan92 = ageSum <= 92;

// console.log(`A soma entre o 1º e o 3º item de "ages" é menor ou igual a 92. Essa afirmação é: ${isLowerThan92}.`)
...
const eigthIsInArray = evenNumbers.some((evenNumber) => evenNumber === 8);

// console.log(eigthIsInArray);