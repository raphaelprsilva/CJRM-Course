...
// let randomArray = ['Coxinha', null, 'Brigadeiro'];

// let newBrFoods = brazilianFoods.concat(randomArray);

// console.log(newBrFoods);
...
const lastItemDogNames = dogNames.length - 1;
// console.log(dogNames[lastItemDogNames]);
...
// Remove os dois elementos do array, para a resolução do exercício
const deleteLilica = dogNames.splice(2, 1);
const deleteMatilda = dogNames.splice(2, 1);

dogNames.forEach(dog => {
  const lastTwoItemCharacters = dog.length - 2;
  const slicedItem = dog.slice(lastTwoItemCharacters);

  dessert += slicedItem;
});

const myFavoriteDessert = dessert.replace('n', 'd');

// console.log(`A ${myFavoriteDessert} é um doce à base de coco, tradicional na América Latina e em Angola.`);
